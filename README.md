...
v1.11.0 中，UTA 的 Planner 可根据任务、约束和实际工具目录生成结构化计划，Router 只能选择已注册工具声明的 action 和参数契约。每条步骤成功标准会保存校验来源与证据，失败后带入 Replan；信息不足或复杂计划需要确认时，桌面端会持久化稳定 request id，可在应用重启后继续原任务。模型输出无效时会回退到确定性规则，高风险工具仍必须人工授权。自动化、25 条离线评测、真实桌面、macOS 重打包、签名和 ZIP 完整性门禁均已通过，v1.11.0 已正式发布。
